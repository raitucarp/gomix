	...
	return func(s *style) styleProp {
		if len(value) == 1 {
			return &properties{
				...
				string(transitionTimingProp):   s.theme.UseVarKey(themes.Custom, "default-transition-timing-function"),
				string(transitionDurationProp): s.theme.UseVarKey(themes.Custom, "default-transition-duration"),
			}
		}
		...
		return &properties{
			string(transitionPropertyProp): fmt.Sprintf("color, background-color, border-color, outline-color, text-decoration-color, fill, stroke, %s, %s, %s, opacity, box-shadow, transform, translate, scale, rotate, filter, -webkit-backdrop-filter, backdrop-filter, display, content-visibility, overlay, pointer-events",
				s.theme.UseVarKey(themes.Custom, "gradient-from"),
				s.theme.UseVarKey(themes.Custom, "gradient-via"),
				s.theme.UseVarKey(themes.Custom, "gradient-to"),
			),
			string(transitionTimingProp):   s.theme.UseVarKey(themes.Custom, "default-transition-timing-function"),
			string(transitionDurationProp): s.theme.UseVarKey(themes.Custom, "default-transition-duration"),
		}
...
func TransitionColors() ApplyProp {
	return func(s *style) styleProp {
		return &properties{
			string(transitionPropertyProp): fmt.Sprintf("color, background-color, border-color, outline-color, text-decoration-color, fill, stroke, %s, %s, %s", s.theme.UseVarKey(themes.Custom, "gradient-from"), s.theme.UseVarKey(themes.Custom, "gradient-via"), s.theme.UseVarKey(themes.Custom, "gradient-to")),
			string(transitionTimingProp):   s.theme.UseVarKey(themes.Custom, "default-transition-timing-function"),
			string(transitionDurationProp): s.theme.UseVarKey(themes.Custom, "default-transition-duration"),
		}
	}
